...
import {moomuekkungDb} from '../../db/restCoupon.js';
const Dashboard = (props) => {
  ...
  useEffect(() => {
        console.log(moomuekkungDb);
  }, []);

  const restBrand = moomuekkungDb[0].member_coupon_list.map((coupon,k) =>
  ...
  const couponUsed = moomuekkungDb[0].member_coupon_use.map((couponUse,k) =>
  <tr key={k+Math.random()}>    
    <td>{k+1}</td>
