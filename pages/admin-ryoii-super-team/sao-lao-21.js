...
const Dashboard = (props) => {
  ...
  useEffect(() => {
        console.log(saoLaoDb);
  }, []);

  const restBrand = saoLaoDb[0].member_coupon_list.map((coupon,k) =>
  ...
  const couponUsed = saoLaoDb[0].member_coupon_use.map((couponUse,k) =>
  <tr key={k+Math.random()}>    
    <td>{k+1}</td>
...
Dashboard.getInitialProps = async ctx => {
  try {
    //   //const res = await axios.get('/api/restaurant-brand');
        const restName = {name:saoLaoDb[0].brand_name}
       return {restName};
     } catch (error) {
      return { error };
     }
