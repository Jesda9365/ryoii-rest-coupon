...
const Dashboard = (props) => {
  ...
  const restBrand = hongminDb[0].member_coupon_list.map((coupon,k) =>
  ...
  const couponUsed = hongminDb[0].member_coupon_use.map((couponUse,k) =>
  <tr key={k+Math.random()}>    
    <td>{k+1}</td>
...
Dashboard.getInitialProps = async ctx => {
  try {
    //   //const res = await axios.get('/api/restaurant-brand');
        const restName = {name:hongminDb[0].brand_name}
       return {restName};
     } catch (error) {
      return { error };
     }
